...
TODO: Present SimGrid models briefly.
...
TODO: Redact this part better.

Not many projects exist on Kubernetes simulation or have been disclosed. We
were able to find only two projects that propose simulations of Kubernetes
clusters.

JoySim \cite{joysim} is a fully fledged Kubernetes simulator. Simulations are
based on synthetic events which means there is no need for any advanced models
to compute resource jobs resource usage and inter dependencies: everything is
pre-written. The strength of this simulator is that the nodes are very light
weight enabling for scalable simulations. JoySim is aimed at studying
scheduling performances and quality in complex scenarios by generating metrics
such as cpu and disk usage and scheduling time. Although, to the best of our
...
The other project is an end of studies project like this one.
k8s-cluster-simulator\cite{k8s-cluster-simulator} is an open source cluster for
evaluating schedulers. Its models are somewhat basic: Pods are submitted with
certain resource requirements, for a certain amount of time which makes it
event based like JoySim. One can provide any scheduler implementation they want
as long as they do so through their Go interface, reducing the amount of
schedulers that can be tested to Go implementations only (or provide an
adaptive layer to their scheduler to support the simulator).\\

TODO: Other simulators for studying schedulers: Alea (Gridsim), Accasim.

TODO: Simulation in HPC (off line vs on line) and how this approaches are different from our fulyl specified workloads.

TODO: It is interesting to look at the other projects based on SimGrid
(SimBOINC and SimGrid-BOINC, WRENCH). They prove the versatility of SimGrid: it
make sno doubt that SimGrid can accurately simulate any Kubernetes cluster.
...
Simulators based on simgrid: SimBOINC \cite{kondo2007simboinc}
