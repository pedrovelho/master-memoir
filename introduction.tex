...
\highlight{The goal of this paragraph is to say that doing real scheduling experiments is hard, this is also hard because the outcome of these experiments does not add value directly to the bussiness itself, so it is difficult to justify such cost. Instead people go along allocating one guy to do the performance analysis.}
...
can be extreme : a HPC job spanning months on a real system can be \highligh{simulated?} in
...
expect the same results.\highlight{Well written paragraph, good job!}\\
...
\highlight{Good point, it is somehow not the real deal, howe can we be sure?}
...
\highlight{Other researchers asserted simgrid accuracy under complex network topollogies workloads, ME. This is also something that proves simgrid maturity}
...
\highlight{Look forward to that!!!}
