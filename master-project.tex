...
be it Big Data centers to store and analyze data, HPC to run simulations or and
models for research or the industry, or GPU banks for machine learning and
crypto-currency mining.  However studying those infrastructures extensively,
for example in order to determine the size of the required system or choose
optimal scheduling policies, is much more challenging.  As these computers
reach scales in the order of warehouses\cite{barroso2018datacenter} quantifying
a system's performance under varying loads, applications, scheduling policies
and system size quickly becomes tremendously hard.\\
...
entire experiment campaign on a real system is impossible for most due to large
overhead and consequent costs both in time and money. With simulation, The gain
in time and spent energy can be quite extreme : a HPC job spanning months on a
real system can be resolved in a matter of minutes, on any domestic computer.
...
their first iteration. With simulation, one can recreate the same conditions in
an experiment anywhere they want, and expect the same results.\\
...
fitfalls\cite{poquet:tel-01757245}. Very often simulators are implemented at
...
life-cycle and implements scheduling policies.} to validate the algorithms, and
thus are strongly coupled together. Consequently, those simulators are not
usable with any other algorithm and are either shipped with the software itself
or worst, are never released and discarded at the end of the development
process. Moreover, still according to  \cite{poquet:tel-01757245}, strong
coupling may lead to unrealistic models, because then cluster resources can be
accessed with ease by the scheduler resulting in it having very precise
information about the system to take decisions, all that at a high frequency.
This conflicts with the real world as a scheduler may not have access to all
the information it wants, and also can suffer from latency when getting it from
the system.

To try and assess these issues and many others not described in this
introduction, a team of researchers at the LIG developed
Batsim\cite{dutot:hal-01333471} which is a general purpose infrastructure
simulator with modularity and separation of concerns in mind.
...
%of  a collection  of  inter-connected  and  vitalized  computers  that  are
...
%applications and is now invading the world of Machine Learning. %TODO : a little reference here?
