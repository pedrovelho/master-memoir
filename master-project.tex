...
Then, in order to extensively experiment on a given system, there are 3 options
left as described in \cite{legrand2015scheduling}: \textit{in vivo},
...
%The other option is
%running experiments on real systems. However, it is expensive and poses major
...
%It is still an active area of research where some
...
Related projects : SimGrid, GridSim, CloudSim (to cite the most important).

Other simulators in unrelated domains: SimBA (volunteer computing), PeerSim,
OverSim (peer to peer), WRENCH (workflows).

Kubernetes simulation: k8-cluster-simulator, joySim.

discontinued projects : GSSIM, Simbatch


\section{Batsim, and related projects}
