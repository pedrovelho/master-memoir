...
\begin{figure}[h]
	...
	\end{subfigure}
	\caption{Effect of minimum delay on the simulation}
...
here is 10ms.  The issue was resolved with an update of the scheduler : the
success rate flattens out at 1 -- or around 1. Then, with earlier versions of
the scheduler, the user may have to adjust the minimum delay in order to run
simulation smoothly. 
...
direct correlation between \textit{minimum delay} and simulation time. It
follows that the best choice for the \textit{minimum delay} now is zero, and we
will use this value for the rest of the experiments.
...
timeout vs makespan: scatter plot, plot a dotted line showing the emulated
value. Same for timeout vs mean\_waiting\_time.
