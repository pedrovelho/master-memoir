...
simulator and the scheduler. We want to verify to want extent time manipulation
...
choose to validate the simulator against an emulated cluster run in
containers.\\

In these next sections, we study the simulator results against three parameters
across three workloads using the default kubernetes scheduler. Note that any
other scheduler can be used, implying some work is done on the API to support
their features.

\begin{itemize}
	\item A \textit{basic} workload, where 200 delays of value 160 are
		submitted at time zero.
	\item A \textit{spaced} workload, where 200 delays of value 160 are
		submitted every 10 seconds.
	\item A \textit{realistic} workload, which is extracted from a workload
		from a real system.
\end{itemize}

\section{Testing against an emulated cluster}

\subsubsection{Protocol}

TODO: validation against k3s, scripts to create a cluster with k3s, scripts to
process swf files, evalys. Include illustration of the protocol.

Define \textit{accuracy}
...
The objective here is to fine tune the simulator parameters in order to find a
compromise between accuracy and scalability.
...
\subsubsection{Minimum delay}

We observed that not leaving enough time to the scheduler each cycle causes it
to put itself in a deadlock state. From there, no decision is made. The
\texttt{--detect-scheduler-deadlock} option allows to exit the simulation with
code 1, to effectively detect such crashes. We expect to see a very high
percentage of crashes when this value is low, that will decrease until there is
enough time for the scheduler to make its computations correctly.\\

TODO: graph - delay vs crash rate

\subsubsection{Maximum simulation time step}
...
\subsubsection{Parameters inter dependency}
